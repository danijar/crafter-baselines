...
class Logger:
    ...
    def create_wights_folder(self):
        (self.logdir / "Models").mkdir(exist_ok=True, parents=True)

    def log_params(self):
        with SummaryWriter(str(self.logdir / "Logs")) as writer:
    ...
    def save_params(self, episode, iteration):
        torch.save({"current_policy_state_dict": self.brain.current_policy.state_dict(),
                   ...
                   str(self.logdir / "Models" / "params.pth"))

    def load_weights(self):
        model_dir = self.logdir.glob("Models/*")
        model_dir.sort()
        checkpoint = torch.load(str(model_dir[-1] / "params.pth"))
        self.logdir = model_dir[-1].split(os.sep)[-1]
        return checkpoint
